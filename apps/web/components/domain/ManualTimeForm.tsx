...
import { logTimeAction, type ActionResult } from '@/actions/time';
...
export function ManualTimeForm({ onCancel }: { onCancel?: () => void }) {
  const t = useT('time.form');
  const tErr = useT('time.errors');
  const tTimer = useT('time.timer');

  const [state, formAction, pending] = useActionState(logTimeAction, initial);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-3" ref={wrapRef}>
      ...
      {state && state.ok && state.data?.flag === 'OVERLAPPING' ? (
