...
describe('teamly smart-table mapping', () => {
  ...
  it('detects table spaces by non-empty schemaProperties', () => {
    expect(isTableSpace({ id: 's', title: 't', description: null })).toBe(false);
    expect(isTableSpace({ id: 's', title: 't', description: null, schemaProperties: [] })).toBe(false);
    expect(isTableSpace({ id: 's', title: 't', description: null, schemaProperties: [{ code: 'c', type: 'text' }] })).toBe(true);
  });
