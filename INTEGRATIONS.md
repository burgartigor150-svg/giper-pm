...
У giper-pm нет общего бота организации. Каждый PM создаёт собственного
бота в [@BotFather](https://t.me/BotFather) и подключает токен через
веб-интерфейс — `/integrations/telegram`. Токен шифруется AES-256-GCM
ключом `TG_TOKEN_ENC_KEY` и хранится в `UserTelegramBot`. Сервис
`apps/tg-bot` (multi-bot runner) поднимает long-polling по каждой
активной записи; реакция на изменения через Redis pub/sub
`tg:bots:reload` + safety-таймер 60 c.

### Подключение

1. PM открывает `/integrations/telegram` (видно только ADMIN/PM).
2. В @BotFather: `/newbot` → имя/username → копирует токен.
3. В @BotFather: `/mybots` → Bot Settings → **Group Privacy → Off**
   (без этого бот не видит обычные сообщения).
4. Вставляет токен в форму. Сервер валидирует через `getMe`,
   шифрует, пишет `UserTelegramBot`, публикует reload — runner
   моментально поднимает нового бота.
5. Добавляет своего бота в групповой чат проекта.
6. Кнопкой «Сгенерировать код» получает `/linkproj TG-XXXXXX` и
   отправляет в группу. Код проверяется на стороне runner-а:
   `botId == owningBot.id && userId == owningBot.userId`.
7. После привязки сообщения чата сохраняются в
   `TelegramProjectMessage`. Кнопкой «Собрать в задачи» (или командой
   `/harvest [N]` в чате) создаются `Task` — по одной на сообщение.

### Команды (только то, что есть)

| Команда                    | Где                       | Действие                                       |
|----------------------------|---------------------------|------------------------------------------------|
| `/start`, `/help`          | в личке бота / в группе    | приветствие, шпаргалка                         |
...
Личные команды (`/me`, `/today`, `/week`, `/log`, `/stop`, `/pair`) и
Telegram Mini App **удалены**: они требовали единого org-бота.
