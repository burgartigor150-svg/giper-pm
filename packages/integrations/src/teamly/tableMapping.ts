...
/** System property codes — auto-managed metadata, not user table columns. */
const SYSTEM_CODES = new Set(['author', 'executor', 'executionDate']);

/** A space is a smart table iff it exposes any schema properties (columns). */
export function isTableSpace(sp: TeamlySpace): boolean {
  return Array.isArray(sp.schemaProperties) && sp.schemaProperties.length > 0;
}
